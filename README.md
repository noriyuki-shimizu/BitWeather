...
* Nodeの環境について<br>
    * Node.jsやnpmなどがローカル環境に存在しない場合、以下のコマンドをプロジェクトのあるディレクトリ上で実行すると、環境構築を行ってくれます。<br>
      環境構築は`brew`コマンドが使用できることが前提となっていますので、コマンドが存在しない場合は、あらかじめインストールを行っておいてください。<br>
      また、`nodebrew`を使用した環境構築ですので`curl`などを用いてインストールなどをしたい場合は、実行しないでください。<br>

    ```
    $ bash setup.sh
    ```
    
    * 上記コマンドは`package.json`に記載されている`dependencies`のパッケージ情報もインストールします。<br>
...
本ツールにおけるメイン実行ファイル(app.1h.js)に実行権限がない場合は、プロジェクトのあるディレクトリ上で以下のコマンドを実行します。
```
$ chmod +x app.1h.js
```
* シンボリックリンクを作成する<br>
